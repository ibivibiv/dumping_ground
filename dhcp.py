...
from pypxe import config


TYPE_53_DHCPDISCOVER = 1
TYPE_53_DHCPREQUEST =  3

class NoLeasesError(Exception):
    pass
...
class DHCPD:
    ...
    def next_ip(self):
        ...
        mac = self.get_mac(self.client_mac)

        ip = None

        try :

            ips = netbox_client.get_ips_by_mac_vrf(config.NB_URL, config.NB_TOKEN,
                                               mac, config.CONTEXT)
        ...
        if ip is None :
            raise NoLeasesError('No IP addresses to lease!')
